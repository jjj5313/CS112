/* File: Grapher.java
 * Contains: public class Grapher, JCanvas
 * Author: Wayne Snyder
 * Date: 10/12/13
 * Class: CS 112, Fall 2013
 * Purpose: This program is a simple graphing class based on the JCanvas class; it 
 ...
 *                                                                     //      center at (x,y)
 *                  public void initCurve(int x, int y);               // start drawing curve at (x,y)
 *                  public void nextCurve(int y, int y);               // draw straight line from last
 *                                                                     // pt to this pt, and reset last pt
 *                  public void drawCurve(int [] A, String s);         // A is of form [x1, f(x1), ..., xn, f(xn)]
 *                                                                     // and these points will be connected by curve
 *                                                                     // and a label of the same color will be written.
 * Associated classes:  JCanvas (below in this file)
 * Known bugs:  None (yet!)
 */
...
public class Grapher {
   ...
   public static void main(String [] args) {
      ...
      Grapher G = new Grapher("Analysis of Algorithms Grapher"); 
      
      
      // the following is a typical curve, pairs of numbers x, f(x)....
      // can also just do integers
      int [] A = new int[40];
      int [] B = new int[40];
      int [] C = new int[40];
      int x = 5; 
   ...
   private final double xScale = 6.74;         // how many pixels per unit on x axis?
   private final double yScale = 0.63;         // on y axis?
   
   private final int xTick = 10;              // how many units on x axis between each tick mark
   private final int yTick = 100;              // on y axis? 
   
   private final String YLabel = "# Comparisons"; 
   private final String XLabel = "Size of Input";
   ...
   public Grapher(String title) {
      ...
      canvas.drawString(YLabel, 15,topMargin/2-5); 
      canvas.drawString(XLabel, width/2-20,height-bottomMargin/2); 
   ...
   // draw a curve by placing successive lines on graph
   // call first with initCurve( x, f(x) ) then continue with nextCurve( x, f(x) ) 
   ...
   public void nextCurve(int x, int y) {  
      canvas.drawLine(x(initX),y(initY), x(x), y(y));
      initX = x; initY = y;
   }
   
   public void initCurve(double x, double y) {
      initX = (int)Math.round(x); initY = (int)Math.round(y);
   }
   
   public void nextCurve(double x, double y) { 
      int x1 = (int) Math.round(x); 
      int y1 = (int) Math.round(y); 
      canvas.drawLine(x(initX),y(initY), x(x1), y(y1));
      initX = x1; initY = y1;
   }
   
   private Color [] colors = { Color.blue, Color.red,  Color.green, Color.black, Color.pink,
      Color.orange, Color.cyan, Color.magenta, Color.darkGray  }; 
   private int cNumber = 0; 
   private int labelX = 80;
   private int labelY = 1000; 
   
   public void drawCurve(int [] A, String label) {
      setPaint(colors[cNumber]);
      cNumber = (cNumber + 1) % colors.length; 
      if(A.length >= 4) {
         initCurve(0, 0); 
    ...
    public void drawCurve(int [] A) {
      drawCurve(A,"Analysis of Algorithms"); 
   }  
   
   public void drawCurve(double [] A, String label) {
      setPaint(colors[cNumber]);
      cNumber = (cNumber + 1) % colors.length; 
      if(A.length >= 4) {
         initCurve(A[0], A[1]); 
   ...
   public void drawCurve(double [] A) {
      drawCurve(A,"Analysis of Algorithms"); 
   }
   
   public void labelCurve(String label) {
      drawString(label, initX, initY+1); 
   }
...
//
//
class JCanvas extends JComponent implements Printable
{
