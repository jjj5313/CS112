      ...
      if(cmd == 1) {
        ...
        for(int i = 0; i < line.length(); ++i) {
          ...
          if(Character.isSpaceChar(cmd2))                  // if space, skip it
            ...
            temp.push(Character.getNumericValue(cmd2));
            ...
            if(temp.isEmpty()){
              reportError("The stack is empty; Action will lead to an underflow error!"); 
            }
            else {
              int first = temp.pop();
              int second = temp.pop();
        ...
        if(!temp.isEmpty()){
          reportError(" Numbers are still in the Stack "); 
        }
